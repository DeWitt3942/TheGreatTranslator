def self.translate_words(words)
      ...
      def self.process_recomended_data(urls)
        ...
        puts 'in hea'
         urls.each do |u|
          ...
          puts t
          # u[:recomended_translation] = JSON.parse(u['content']).text
          # u[:recomended_translation] = "none"
          # u.delete('content')
          # u.delete('link')
        end
      end
      ...
      puts 'Next step!'
      # add recomended translations

      make_recomended_links urls
      # puts 'S4'
      load_pages(urls, :process_recommended_data)
      puts 'S5'
...
IO.readlines(path).each do |line|
  if line.strip == ''
    ...
    ts = ts.map do |time|
      res = ""
      time = time.split(':')
      k_digits(((time[0].to_i*60 + time[1].to_i).to_s), 2) + ':'+ k_digits(time[2], 2)+'.' + k_digits(time[3], 3)
    end
    times[current_id] = ts[0] + ' --> ' + ts[1]
    text[current_id] = ""
    state = 2
  elsif state==2 
    text[current_id] += line.strip + '<br>'
  end
end
result = []
